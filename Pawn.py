...
class Pawn(Piece):
    ...
    def get_possible_moves(self, board, position):
        ...
        capture_directions = np.array([[-1, 1], [1, 1]]) if self.color == 'white' else np.array([[-1, -1], [1, -1]])
        
        for d in capture_directions:
            new_pos = position + np.array([direction, d[0]])
            if np.all((0 <= new_pos) & (new_pos < 8)):
                if board[new_pos[0], new_pos[1]] is not None and board[new_pos[0], new_pos[1]].color != self.color:
                    moves.append(tuple(new_pos))
        ...
        for d in [-1, 1]:
            ...
            if 0 <= new_pos[1] < 8:
                if isinstance(board[position[0], new_pos[1]], Pawn):
                    if board[position[0], new_pos[1]].color != self.color and board[position[0], new_pos[1]].en_passant_target:
